...
#+begin_src clojure
  (require '[dinero.core :as dinero]
           '[dinero.comparison :as comparison]
           '[dinero.formatting :as formatting]
           '[dinero.math :as math]
           '[dinero.parsing :as parsing]
...
#+begin_src clojure
  (let [m1 (dinero/money-of 1234.5678 :eur)
    ...
    (println (formatting/format m1 {:locale germany}))
    (println (formatting/format m1 {:locale germany :symbol-style :code}))
    (println (formatting/format m1 {:locale germany :rounding-mode :down :symbol-style :code}))
    (println (formatting/format m1 {:locale germany :rounding-mode :down :decimal-places 0 :symbol-style :code})))
  ;; 1.234,57 €
  ;; 1.234,57 EUR
  ;; 1.234,56 EUR
  ;; 1.234 EUR
...
#+begin_src clojure
  (let [m1 (dinero/money-of 1234.5678 :eur)
        germany java.util.Locale/GERMANY]
    (println (formatting/format-with-pattern m1 "#,##0.00 ¤" {:locale germany}))
    (println (formatting/format-with-pattern m1 "#,##0.00 ¤¤" {:locale germany}))
    (println (formatting/format-with-pattern m1 "#,##0.000 ¤" {:locale germany}))
    (println (formatting/format-with-pattern m1 "#,##0 ¤" {:locale germany}))
    (println (formatting/format-with-pattern m1 "#,##0 ¤" {:locale germany :rounding-mode :down})))
  ;; 1.234,57 €
  ;; 1.234,57 EUR
  ;; 1.234,568 €
  ;; 1.235 €
...
#+begin_src clojure
  (parsing/parse-containing-iso-4217-symbol "1.234,56 €" java.util.Locale/GERMANY)
  ;; => {:amount 1234.56M, :currency :eur}

  (parsing/parse-containing-iso-4217-symbol "£1,234.56" java.util.Locale/UK)
...
#+begin_src clojure
  (parsing/parse-containing-iso-4217-symbol "1.234,56 EUR" java.util.Locale/GERMANY)
  ;; java.text.ParseException
  ;; Unparseable number: "1.234,56 EUR"

  (parsing/parse-containing-iso-4217-symbol "1,23456789 ₿" java.util.Locale/GERMANY)
  ;; java.text.ParseException
  ;; Unparseable number: "1,23456789 ₿"

  (parsing/parse-containing-iso-4217-symbol "1,23456789 BTC" java.util.Locale/GERMANY)
  ;; java.text.ParseException
...
You could use the following functions to do equality and comparison operations on monetary amounts: ~=~, =not==, =money<=, =money<==, =money>=, =money>==, =money-zero?=, =monmey-pos?=, and =money-neg?=.
...
#+begin_src clojure
  ...
  (let [money (dinero/money-of 0 :eur)]
    (comparison/money-zero? money))
  ;; => true

  (let [money (dinero/money-of -1 :eur)]
    (comparison/money-pos? money))
  ;; => false

  (let [money (dinero/money-of -1 :eur)]
    (comparison/money-neg? money))
  ;; => true
#+end_src
...
#+begin_src clojure
  (let [money (dinero/money-of 1.025 :chf)]
    (formatting/format money {:locale (java.util.Locale. "de" "CH")}))
  ;; => "CHF 1.02"

  (let [money (dinero/money-of 1.025 :chf)
        rounded-money (rounding/round money rounding/chf-rounding-fn)]
    (formatting/format rounded-money {:locale (java.util.Locale. "de" "CH")}))
  ;; => "CHF 1.05"
#+end_src
** License
