...
#+begin_src clojure
  (require '[dinero.core :as dinero]
           ...
           '[dinero.text-format :as text-format])
#+end_src
*** Monetary amounts
...
#+begin_src clojure
  (let [m1 (dinero/money-of 1234.5678 :eur)
    ...
    (println (text-format/format m1 {:locale germany}))
    (println (text-format/format m1 {:locale germany :symbol-style :code}))
    (println (text-format/format m1 {:locale germany :rounding-mode :down :symbol-style :code}))
    (println (text-format/format m1 {:locale germany :rounding-mode :down :decimal-places 0 :symbol-style :code})))
  ;; 1.234,57 €
  ;; 1.234,57 EUR
  ;; 1.234,56 EUR
  ;; 1.234 EUR
...
#+begin_src clojure
  (let [m1 (dinero/money-of 1234.5678 :eur)
        germany java.util.Locale/GERMANY]
    (println (text-format/format-with-pattern m1 "#,##0.00 ¤" {:locale germany}))
    (println (text-format/format-with-pattern m1 "#,##0.00 ¤¤" {:locale germany}))
    (println (text-format/format-with-pattern m1 "#,##0.000 ¤" {:locale germany}))
    (println (text-format/format-with-pattern m1 "#,##0 ¤" {:locale germany}))
    (println (text-format/format-with-pattern m1 "#,##0 ¤" {:locale germany :rounding-mode :down})))
  ;; 1.234,57 €
  ;; 1.234,57 EUR
  ;; 1.234,568 €
  ;; 1.235 €
...
#+begin_src clojure
  (let [money (dinero/money-of 1.025 :chf)]
    (text-format/format money {:locale (java.util.Locale. "de" "CH")}))
  ;; => "CHF 1.02"

  (let [money (dinero/money-of 1.025 :chf)
        rounded-money (rounding/round money rounding/chf-rounding-fn)]
    (text-format/format rounded-money {:locale (java.util.Locale. "de" "CH")}))
  ;; => "CHF 1.05"
#+end_src
** License
